...
`max_items` を指定すると1回の実行で確認する件数を変えられる（既定10件）。一覧が新着順でなく常設記事が上位を占めるサイトでは、多めにしないと新着を取りこぼす。

`scrape` ではhrefのクエリ・フラグメントを落としてから照合・重複判定を行うため、`?utm_campaign=...` 付きのリンクも同一記事として扱われる。
...
既読記録（`seen_*.json`）と差分検知ハッシュの保存は**メール送信に成功してから**行う。先に保存すると、SMTP障害や番犬（`run.sh` の8分制限）による強制終了時に「配信済み扱いだが届いていない」記事が生まれ、二度と配信されなくなるため。
